...
class ED:
    ...
    def add_level(self, energy, bottom_text='', position=None, color='k',
                  top_text='', right_text='', left_text='Energy', occupied=False):
        '''
        Method of ED class
        ...
        if occupied:
            self.left_texts.append(str(left_text))
            if position == 0:
                self.bottom_texts.append("\u2b06")
            elif position == 1:
                self.bottom_texts.append("\u2b07")
        else:
            # self.top_texts.append(str(top_text))
            self.left_texts.append(str(left_text))
            self.bottom_texts.append("")
    ...
    def plot(self, show_IDs=False, cbm=None, vbm=None):
        '''
        Method of ED class
        ...
        if cbm and vbm:
            ax.hlines(cbm, 0, self.dimension* 2.5, color="red", linestyles="--")
            ax.text(self.dimension*2.55,  # X
                    cbm,  # Y
                    "VBM {}".format(cbm),  # self.top_texts
                    horizontalalignment='left',
                    verticalalignment='center')

            ax.hlines(vbm, 0, self.dimension * 2.5, color="red", linestyles="--")
            ax.text(self.dimension*2.55,  # X
                    vbm,  # Y
                    "CBM {}".format(vbm),  # self.top_texts
                    horizontalalignment='left',
                    verticalalignment='center')
        for level in data:
